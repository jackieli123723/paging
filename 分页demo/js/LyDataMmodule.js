(function($){
      $.fn.showTips=function(options){
        var defaults={
                imgSrc:"images/app/app-sure-del.png",//默认图片
                titleTxt:"你确定要删除吗?",//大标题提示语
                titleTxtSmall:"此条信息将会被永久删除",//
                cancelTxt:"取消",//取消提示语
                sureTxt:"删除",//确定提示语
                callBackSure: function() {},//确定回掉函数
                callBackCancel: function() {}, //取消回掉函数
                };
        var defaultser=$.extend(defaults,options);
        return this.each(function(){                  
            var html="";
            html+=' <div class="del-confirm-box" style="display:block;">'
            html+=' <div class="del-confirm-text">' 
            html+=' <img src="'+defaultser.imgSrc+'" />' 
            html+=' <span>'+defaultser.titleTxt+'</span>' 
            html+=' <span class="forver">'+defaultser.titleTxtSmall+'</span>' 
            html+=' </div>'              
            html+=' <div class="del-confirm-btn">'  
            html+=' <span class="del-confirm-sure">'+defaultser.sureTxt+'</span>' 
            html+=' <span class="del-confirm-cancel">'+defaultser.cancelTxt+'</span>' 
            html+=' </div>' 
            html+='</div>'            
            $("body").append(html);             
            $(".del-confirm-cancel").on("click",function(){ 
                 defaultser.callBackCancel();                 
                $(this).parent().parent().remove();
            })
            
            $(".del-confirm-sure").on("click",function(){
                 defaultser.callBackSure();
                 $(this).parent().parent().remove();
            })
        
        })
            
      };
 
        $.show_msg=function(msg, timeOut) {
          var timer = null;
          var c = $("#msg_dis_container");
          timeOut = timeOut || 1000;
          if(c.length==0){
            $(document.body).append('<div class="motify" style="display:none" id="msg_dis_container"><div class="motify-inner" id="msg_dis_content"></div></div>');
            c = $("#msg_dis_container");
          }
          $("#msg_dis_content").html(msg);
          c.css("display","block");
          clearTimeout(timer);
          timer = setTimeout(function(){
            c.css("display","none");
          },timeOut);
        };

        $.successTip=function(msg, timeOut) {
          var timer = null;
          var c = $("#successTip");
          timeOut = timeOut || 1000;
          if(c.length==0){
            $(document.body).append('<div class="my_tip tip_success" id="successTip" style="display:none"><div><span></span></div></div>');
            c = $("#successTip");
          }
          $("#successTip span").html(msg);
          c.css("display","block");
          clearTimeout(timer);
          timer = setTimeout(function(){
            c.css("display","none");
          },timeOut);
        };

        $.errorTip=function(msg, timeOut) {
          var timer = null;
          var c = $("#errorTip");
          timeOut = timeOut || 1000;
          if(c.length==0){
            $(document.body).append('<div class="my_tip tip_error" id="errorTip" style="display:none"><div><span></span></div></div>');
            c = $("#errorTip");
          }
          $("#errorTip span").html(msg);
          c.css("display","block");
          clearTimeout(timer);
          timer = setTimeout(function(){
            c.css("display","none");
          },timeOut);
        };

    
})($);


/*cs-data-web组件开发*/
Function.prototype.curry = function () {
    var slice = Array.prototype.slice,
        args = slice.apply(arguments),
        that = this ;
    return function () {
        return that.apply(null , args.concat(slice.apply(arguments)));
    };
}; 
/*当循环执行的时候代替setTimeout,性能优化*/
window.requestAnimFrame = (function() {
    return window.requestAnimationFrame || window.webkitRequestAnimationFrame || window.mozRequestAnimationFrame || window.oRequestAnimationFrame || window.msRequestAnimationFrame ||
        function(callback, element) {
            return window.setTimeout(callback, 1000 / 60);
        };
})();
//阻止默认行为
function stopProp(e){
    e.preventDefault();
    e.stopPropagation();
}

/******* 2016.07.15*******/
/**
 * 动画展开/关闭
 $("Divs").open(anim, time);
 $("Divs").close(anim, time, des);
 anim: 动画包括：
     渐入 fadeAnim,(默认动画)
     弹出 popAnim,
     下滑 slideDAnim,
     上滑 slideUpAnim,
     右滑 slideRightAnim
     左滑 slideRightAnim
 time: 是动画时间
 des: 是否销毁
 */
(function($){
    $.fn.open = function(anim, time){
        var _this = $(this);
        
        if(_this.hasClass("hidden")){ 
            window.requestAnimFrame(_this.open.curry(anim, time));
        }else{
            time && _this.css("-webkit-animation-duration", time + "ms");
            anim = anim || "fadeAnim";
            _this.show().addClass(anim);
            _this.addClass("shown").animate(function () {
                _this.removeClass(anim);
                _this.removeClass("shown");
            });
        }
    };
    $.fn.close = function(anim, time, des){ 
        var _this = $(this);
        if(_this.hasClass("shown")){ //如果显示动画还未执行完毕，过300ms再执行
            window.requestAnimFrame(_this.close.curry(anim, time));
        }else{
            time && _this.css("-webkit-animation-duration", time+"ms");
            anim = anim || "fadeAnim";
            _this.addClass(anim);
            _this.addClass("hidden").animate(function(e){
                _this.hide();
                _this.removeClass(anim).removeClass("hidden");
                des && _this.remove();
            });
        }
    };
})($);


/**
 *加载动画
 *text 加载文字
 *anime 出现的动画
 *$.showLoad('text', 'anime'); //显示
 *$.hideLoad(); //关闭
 */
(function($){
    var load;
    $.showLoad = function(text, animte) { //开启加载
        load = load || initLoad();
        load.text.html(text || "正在加载");
        load.div.open(animte, 150);
        $.Load = load.div;
    },
    $.hideLoad = function(animte) { //结束加载
        load && load.div.hide();
    }
    function initLoad() { //生成加载层
        var c = document.createElement("div"),
            b = document.createElement("span"),
            chtml = "" ;
        c.style.cssText = "z-index:11000; position:fixed; top:0; left:0; width:100%; height:100%; overflow:hidden; background: rgba(0,0,0,.1); text-align:center; display:none;";
        chtml = "<div style='min-width: 4rem; color: #fff; font-size: .8rem; margin: -5rem auto 0; position:relative; top:50%; -webkit-border-radius: .3rem; border-radius: .3rem; background: rgba(0, 0, 0, 0.6); display: inline-block; padding: .5rem .8rem;'>";
        chtml += "<img style='width:2rem;margin:.6rem auto; display:block;' src = 'data:image/gif;base64,'/>";
        chtml += "</div>";
        c.innerHTML = chtml;
        c.firstChild.appendChild(b);
        document.body.appendChild(c);
        return {
            div: $(c),
            text: $(b)
        }
    }
})($);

